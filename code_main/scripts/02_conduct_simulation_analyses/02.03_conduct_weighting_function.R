...
conduct_one_weighting <- function(df_samp) {
  ...
  fit_ps_weighted <- glm(insomnia ~ bmi + age + visit_years, weights = final_weights, family = binomial(), data = df_samp)
  # based on logistic regression using OSW as covariate
  fit_ps_cov <- glm(insomnia ~ bmi + age + visit_years + final_weights, family = binomial(), data = df_samp)
