...
conduct_cem <- function(df_samp) {
  ...
  df_matches_cem_cov <- matchit(
    insomnia ~ bmi_cat + age_cat + visit_years_cat,
  ...
  df_matches_cem_weighted <- matchit(
    insomnia ~ bmi_cat + age_cat + visit_years_cat + final_weights_cat,
    method = "cem", # CEM matching
    estimand = "ATT", 
    data = df_samp
  ) %>% 
  ...
  # combine results
  results <- list(df_matches_cem_cov, df_matches_cem_weighted)
  return(results)
}
