...
# create function to calculate propensity scores (for weighting) and obtain matches (for psm)
conduct_one_psm <- function(df_samp) {
  # based on weighted logistic regression using OSW
  # obtain matchit object 
  matchit_psm_weighted <- matchit(
    insomnia ~ bmi + visit_years,
    ...
    s.weights = ~ final_weights, # use weighted regression
    data = df_samp
  )
  ...
  matchit_psm_cov <- matchit(
    insomnia ~ bmi + visit_years + final_weights,
    method = "nearest", # nearest neighbor matching 
    distance = "glm", # use propensity scores obtained from logistic regression
    estimand = "ATT", # controls are selected to be matched w/ treated 
    data = df_samp
  ...
  # combine results
  results <- list(df_matches_psm_weighted, df_matches_psm_cov)
  return(results)
}
