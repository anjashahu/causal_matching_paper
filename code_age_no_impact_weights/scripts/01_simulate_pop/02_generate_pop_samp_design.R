...
# stratify households by hispanic surname/no hispanic surname
df_pop_hh <- df_pop_hh %>%
  group_by(strata) %>%
  # sample different percentages of HH based on strata and assign those as Hispanic surname
  sample_frac(size = case_when(
    strata == 1 | strata == 2 | strata == 3 | strata == 4 ~ 0.25,
    strata == 5 | strata == 6 | strata == 7 | strata == 8 ~ 0.35
  )) %>%
  mutate(hh_hisp_surname = 1) %>%
  ungroup() %>%
  select(hh, hh_hisp_surname) %>% 
  # rejoin the HHs sampled as Hispanic surname to the entire HH data frame
  right_join(df_pop_hh, by = "hh") %>% 
  mutate(hh_hisp_surname = if_else(is.na(hh_hisp_surname), 0, 1)) %>%
  arrange(hh) %>%
  select("bg", "strata", "bg_size", "bg_sampling_prob", "hh", "hh_size", "hh_mean_age", "hh_hisp_surname")

# check to make sure stratification by hispanic surname was done properly
#df_pop_hh %>% 
  #group_by(strata, hh_hisp_surname) %>%
  #count() %>%
  #group_by(strata) %>%
  #mutate(total = sum(n)) %>%
  #ungroup() %>%
...
# add HH sampling probabilities stratified by hispanic surname status
df_pop_hh <- df_pop_hh %>%
  mutate(
    hh_sampling_prob = case_when(
      (strata == 1 | strata == 2) & hh_hisp_surname == 1 ~ 0.10,
      (strata == 1 | strata == 2) & hh_hisp_surname == 0 ~ 0.05,
      (strata == 3 | strata == 4) & hh_hisp_surname == 1 ~ 0.14,
      (strata == 3 | strata == 4) & hh_hisp_surname == 0 ~ 0.04,
      (strata == 5 | strata == 6) & hh_hisp_surname == 1 ~ 0.09,
      (strata == 5 | strata == 6) & hh_hisp_surname == 0 ~ 0.03,
      (strata == 7 | strata == 8) & hh_hisp_surname == 1 ~ 0.08,
      (strata == 7 | strata == 8) & hh_hisp_surname == 0 ~ 0.02
    )
  )

# check to make sure HH sampling probabilities were added correctly
#df_pop_hh %>%
  #group_by(strata, hh_hisp_surname) %>%
  #summarise(mean = mean(hh_sampling_prob))
